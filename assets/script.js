$(document).ready(function () {
	...
	$('#deal-btn').click(function () {
		$.ajax({
			...
			for (var i = 0; i < newDeck.length; i++) {
				...
				if (
					newDeck[i].value != 'KING' &&
					newDeck[i].value != 'QUEEN' &&
					newDeck[i].value != 'JACK' &&
					newDeck[i].value != 'ACE'
				) {
				...
				$.ajax({
					...
					if (mealDisplay.meals[0].strSource == '' || mealDisplay.meals[0].strSource == null) {
						...
						if (mealDisplay.meals[0].strYoutube == '' || mealDisplay.meals[0].strYoutube == null) {
							//if the recipe link isn't valid,  check for a youtube link
	...
	function getHistory() {
		historyData = JSON.parse(localStorage.getItem('historyKEY')) || [
			{ Name: '', Meal: '', Date: '', ImgURL: '', RecipeLink: '' }
		];
	}
	...
	function populateHistory() {
		...
		if (historyData[0].Name) {
			...
			historyData.forEach(function (object) {
				...
				const img = $('<img>')
					.attr('src', object.ImgURL)
					.attr('alt', object.Meal)
					.addClass('past-recipe-img')
					.width('100%');
				...
				const pastRecipeLink = $('<a>')
					.addClass('past-recipe-link')
					.attr('href', object.RecipeLink)
					.attr('target', '_blank')
					.text('View Recipe');
